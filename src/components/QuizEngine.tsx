...
export function QuizEngine() {
  ...
  useEffect(() => {
    const startSession = async () => {
      try {
        const res = await fetch("http://127.0.0.1:8000/api/session/start?user_id=root2", {
          method: "POST",
        });
  ...
  const handleAnswer = async (value: any) => {
    ...
    // ✅ Append to backend session
    if (sessionId) {
      try {
        await fetch("http://127.0.0.1:8000/api/session/append", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            session_id: sessionId,
            question: currentQuestion.question,
            answer: formatAnswerForChat(currentQuestion, value),
          }),
        });
      } catch (error) {
